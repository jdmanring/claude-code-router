...
`402` 或 `429` 响应会让失败的模型目标暂时进入冷却，使后续请求在额度或限流窗口恢复前跳过它。如果上游返回正数的 `Retry-After`，CCR 会使用该时长；没有时默认冷却 60 秒。即使最后一个回退目标仍在冷却，当前请求也会尝试它。

冷却不同于单次请求的回退等待：回退等待只影响当前请求的下一次尝试，冷却则会让后续请求暂时不优先选择同一个目标。trace 和 `ccr-log` 都会显示因冷却而跳过的目标。
